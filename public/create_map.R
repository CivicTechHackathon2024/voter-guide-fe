...
# Read the shapefile
districts <- read_sf(dsn = "districts_shp", layer = "districts_shp") %>% filter(TOWN %in% c("BOSTON"))
# Color each ward
...
leaflet(districts) %>% 
  ...
  addPolygons(
    ...
    popup = ~paste0('<a href=http://localhost:3000/learn_more/', WARD, '/', PRECINCT, '>', TOWN, ": Ward ", WARD, ", Precinct ", PRECINCT),
    fillOpacity = 0.5,
